...
# HEALTH CHECK SCRIPT
# =============================================================================
# Verifies all Ecommerce application services are running correctly.
#
# Usage:
...
# Check Frontend
# -----------------------------------------------------------------------------
if !  check_health "Ecommerce Frontend" "${FRONTEND_URL}"; then
    FAILED=1
fi
...
# -----------------------------------------------------------------------------
if ! check_health "Ecommerce Backend" "${BACKEND_URL}/health"; then
    FAILED=1
fi
...
if command -v docker &> /dev/null; then
    ...
    docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep -E "(ecommerce|NAMES)" || echo "No ecommerce containers running"
fi
...
if [ $FAILED -eq 0 ]; then
    ...
    echo "  1. Check Docker logs:  docker logs ecommerce-frontend"
    echo "  2. Check Docker logs: docker logs ecommerce-backend"
